...
alias tile_size = 16
alias halo = 1
alias shared_tile_size = tile_size + 2 * halo

# Unified stencil computation kernel
fn heat_step[
    layout: Layout
](
    input: LayoutTensor[dtype, layout, MutableAnyOrigin],
    ...
    if gx < width and gy < height:
        if gx == 0 or gx == width - 1 or gy == 0 or gy == height - 1:
            ...
            var down = shared_mem[(sy + 1) * shared_tile_size + sx]
            var center = shared_mem[sy * shared_tile_size + sx]
            output[gy, gx] = r * (left + right + up + down) + (1.0 - 4.0 * r) * center
...
def main():
    ...
    ctx.enqueue_copy[dtype](dst_buf=in_device_buffer, src_buf=in_host_buffer)
    ctx.synchronize()
    ...
    for _ in range(steps):
        ctx.enqueue_function[heat_step[layout]](
            in_tensor,
            out_tensor,
            block_dim=block,
            grid_dim=grid
        ...
        ctx.synchronize()
        ctx.enqueue_copy[dtype](dst_buf=in_device_buffer, src_buf=out_device_buffer)
        ctx.synchronize()

    print("Simulation completed; Steps:", steps)

    ctx.enqueue_copy[dtype](dst_buf=result_host_buffer, src_buf=out_device_buffer)
    ctx.synchronize()
